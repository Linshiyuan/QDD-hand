...
int main(int argc, char **argv) {
  ...
  try {
    ros_ser.setPort("/dev/ttyUSB3");
    ros_ser.setBaudrate(1000000);
...
send2="okok";
  ...
  // std::cout<<std::hex<< buffer[i]<<std::endl;
      ...
      att_x = int16_t((buffer[2] << 8) + (buffer[3])) * 0.0055;
      att_y = int16_t((buffer[4] << 8) + (buffer[5])) * 0.0055;
      att_z = int16_t((buffer[6] << 8) + (buffer[7])) * 0.0055;
